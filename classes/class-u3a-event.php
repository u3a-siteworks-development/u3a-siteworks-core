...
class U3aEvent // phpcs:ignore PSR1.Classes.ClassDeclaration.MissingNamespace
{
    ...
    public static function display_eventlist($atts, $content = '')
    {
        ...
        if (!in_array($layout, ['list', 'grid', 'calendar'])) {
    ...
    public static function display_event_listing($posts, $when, $show_group, $display_args)
    {
        ...
        switch ($display_args['layout']) {
            case 'calendar':
                U3aEvent::build_ics($sortedposts, $show_group, $display_args);
                break;
            case 'grid':
    ...
    private static function build_ics($sortedposts, $show_group, $display_args)
    {
        $filename = wp_get_upload_dir()['basedir'] . "/event_calendars/";
        if (!is_dir($filename)) {
            mkdir($filename, 0755, true);
        }
        if ($display_args['calname'] == '') {
            $filename .= "default_event_calendar.ics";
        } else {
            $filename .= $display_args['calname'] . ".ics";
        }
        ...
        U3aEvent::add_ics_entries($file, $sortedposts, $show_group, $display_args);
        fputs($file, "END:VCALENDAR" . PHP_EOL);
        fclose($file);
        return;
    }

    private static function add_ics_entries($file, $sortedposts, $show_group, $display_args)
    {
        foreach ($sortedposts as $sortedpost) {
            ...
            $datetime = $sortedpost['epochtime'];
            $formatted_date = date("Ymd\THis\Z", $datetime);
            fputs($file, "DTSTART:" . $formatted_date . PHP_EOL);
            $days = get_post_meta($event->ID, 'eventDays', true);
            $endtime = $sortedpost['epochendtime'];
            if ($days >= 1) {
                $endtime += $days * 86400;
            }
            $formatted_date = date("Ymd\THis\Z", $endtime);
            fputs($file, "DTEND:" . $formatted_date . PHP_EOL);
            $permalink = get_the_permalink($event);
            if ($display_args['linkonly']) {
                fputs($file, "DESCRIPTION:" . "<a href=\"" . $permalink .
                    "\">" . $title . "</a>" . PHP_EOL);
            } else {
                $extract = U3aEvent::get_event_extract($event);
    ...
    public function event_date_and_time()
    {
        ...
        if ('' != $time) {
            if ('' != $endtime) {
                ...
                return [
                    'date' => date($dateformat, $tempstart),
