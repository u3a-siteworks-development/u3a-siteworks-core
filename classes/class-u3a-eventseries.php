...
class U3aEventSeries
{
    ...
    const MAX_FREQUENCY_LENGTH = 13; // Twice monthly - need to increase if longer keys added to frequency_list
    ...
    public static function register_eventseries()
    {
        ...
        add_action(
            'admin_menu',
            function () {
                add_submenu_page('edit.php?post_type=u3a_event', 'Add Event Series', 'Add Event Series', 'manage_options', 'post-new.php?post_type=u3a_eventseries');
            }
        );
        // redirect from the Event Series admin list so that doesn't get shown
        add_action(
            'admin_init',
            function () {
                global $pagenow;
                # Check current admin page.
                if ($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'u3a_eventseries') {
                    wp_redirect(admin_url('/edit.php?post_type=u3a_event'));
                    exit;
                }
            }
        );
    ...
    public static function field_descriptions()
    {
        ...
        $fields[] = [
            ...
            'maxlength' => self::MAX_FREQUENCY_LENGTH,  // over-cautious validation check?!
            ];
        $fields[] = [
            'type'    => 'heading',
            'name'    => 'Make sure that the starting dates and frequency meet your needs.',
            'id'      => 'datePattern',
            'desc'    => 'If you choose "Monthly" each event will be on the same day and week of each month. "Twice monthly" events will be in either the 1st and 3rd week or 2nd and 4th week of the month<br>If you choose "Monthly" or "Twice monthly", your start date must not be in the fifth week of a month (29th/30th/31st).',
            ];
        $fields[] = [
            'type'    => 'number',
    ...
    * but not beyond  = edate
    ...
    public static function create_series_of_events($post_id, $post)
    {
        ...
        for ($i = 1; $i <= $num_events; $i++) {
            ...
            $event_title = $post->post_title . '-' . $date_string;
        ...
        wp_update_post(['ID' => $post_id,
                        'post_content' => self::add_default_content('', $post). $eventseries_content,
                       ]);
        return;
    }
    ...
    {
        if ($post->post_type == U3A_EVENTSERIES_CPT) {
            $content = '<!-- wp:paragraph --><p>When you publish this, a series of events will be created.<br>Each event will have a title consisting of the above name followed by the event date.<br>You can then view the series definition which will list the created events and you can edit each one, perhaps giving it an event specific title and adding a description of the event.<br>Note: you cannot create further events from this event series by subsequently editing this eventseries.</p><!-- /wp:paragraph -->  ';
            return $content;
        }
        return $content;
    }
