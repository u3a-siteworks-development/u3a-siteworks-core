...
class U3aContact // phpcs:ignore PSR1.Classes.ClassDeclaration.MissingNamespace
{
    use ChangePrompt;
    use AddMetabox;
    use ManageCrossRefs;
     ...
     */
    public static $xref_meta_key_list = "'coordinator_ID', 'coordinator2_ID',
                         'deputy_ID', 'tutor_ID', 'eventOrganiser_ID'";
    ...
    private const MAX_CONTACT_NAME = 60;
    ...
    public static function initialise($plugin_file)
    {
        ...
        add_action('init', [self::class, 'register_contacts']);
        ...
        add_filter('manage_' . U3A_CONTACT_CPT . '_posts_columns', [self::class, 'change_columns']);
        add_action('manage_' . U3A_CONTACT_CPT . '_posts_custom_column', [self::class, 'show_column_data'], 10, 2);
    ...
    public static function register_contacts()
    {
        $args = [
            ...
            'rewrite'      => ['slug' => sanitize_title(U3A_CONTACT_CPT . 's')],
            'has_archive'  => false,
    ...
    public static function field_descriptions()
    {
        ...
        $fields[] = [
            ...
            'std'  => 'A contact may be generic with a display name, e.g. like "French Leader"<br>' .
                         'Each item of information is optional and should be omitted 
                         if the contact does not want to be contacted by that method.',
        ];
        $fields[] = [
            'type'      => 'text',
    ...
    public static function change_columns($columns)
    {
        ...
        // just changing the displayed name of this column.
        $columns['email'] = 'Email';
        return $columns;
    }
    //end change_columns()
    ...
    public function contact_text()
    {
        ...
        if (current_user_can('edit_others_posts') and !$disabled_top_toolbar) {
            ...
            $edit_HTML = "<a href= '$edit_link'><span style='flex; background-color:yellow;' 
                    class='dashicons dashicons-edit' title='$edit_hint'></span></a>";
        };
